...
std::ostream &operator<<(std::ostream &os, const InterCode &code) {
    switch (code.kind) {
        ...
        case IR_ASSIGN:
            ...
            break;
        case IR_ADD:
            os << code.binOp.result << " := " << code.binOp.op1 << " + " << code.binOp.op2;
            break;
        case IR_SUB:
            os << code.binOp.result << " := " << code.binOp.op1 << " - " << code.binOp.op2;
            break;
        ...
        case IR_GOTO:
            ...
            break;
        case IR_IF_GOTO:
            os << "IF " << code.ifGoto.x << " " << code.ifGoto.relop << " " << code.ifGoto.y << " GOTO "
               << code.ifGoto.z;
            break;
        case IR_RETURN:
...
void IRGenerator::genVarDec(pASTNode vardec, Operand *op) {
    if (vardec->child_count == 1) {
        ...
        if (type.k == Kind::Basic) {
            ...
            auto size = type.size();
            codes.genIRCode(IR_DEC, {Operand(OP_VARIABLE, fd->first), Operand(OP_SIZE, size)});
    ...
    if (exp->child_count == 1) {
        ...
        if (!strcmp(name1, "Exp") && !strcmp(name3, "Exp")) {
            if (!strcmp(name2, "ASSIGNOP")) {
                ...
                codes.genIRCode(IR_ASSIGN, {t1, t2});
            } else if (!strcmp(name2, "AND") || !strcmp(name2, "OR") || !strcmp(name2, "RELOP")) {
                auto label1 = codes.newLabel();
                auto label2 = codes.newLabel();
                auto trueNum = Operand(OP_CONSTANT, 1);
                auto falseNum = Operand(OP_CONSTANT, 0);
...
void IRGenerator::genCond(pASTNode exp, Operand *labelTrue, Operand *labelFalse) {
    // Exp -> Exp AND Exp
    //      | Exp OR Exp
    //      | Exp RELOP Exp
    //      | NOT Exp
