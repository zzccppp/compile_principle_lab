...
struct InterCodeList {
  ...
  friend std::ostream &operator<<(std::ostream &os, const InterCodeList &list) {
    for (const auto &item : list.codes) {
      os << item << std::endl;
    }
    return os;
  }
...
public:
  ...
  void addSymbols(FieldTable &tab) { sym = tab; }
  ...
  inline void printIRCodes() { codes.printIRCodes(); }

  inline void writeIRCodes(std::ostream &os) { os << codes; };
};
